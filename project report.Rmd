---
title: "Consulting Project Report: Exploration of Relationship Between Age and Diversity Score in Orangutan Behavior"
...
To ensure the dataset's usability and reliability, extensive cleaning and preprocessing were conducted. Rows marked for exclusion were removed, as were records with insufficient awake time (Minutes awake \< 360). Missing values in key columns were addressed to maintain consistency, and activity metrics were normalized relative to total awake time to facilitate comparisons across individuals and sessions. Duplicate records were eliminated, and data from juveniles and adults were processed separately before being merged into a single, cohesive dataset.
...
Here we propose three new models building upon the client models, we treat age as a categorical variable, and based on the plots shown in EDA part, sex could be an important variable, we can add interaction term between sex and age, and the variable minutes.awake into the model:
...
```
According to the chi-square test, newmodel.3 is the best model, we can further check the performance of newmodel.3 using AIC and MSE below.
...
```
According to the AIC and MSE comparison above, our models have lower AIC and MSE, indicating that our model performs better. Also, based on this comparison table, among our models, our model 3 have both lowest AIC and MSE, meaning it is the best model.

## Our Model 3 Performance Using Cross Validation
...
## Refit Model 3 Using stan_glmer To Validate Performance
```{r, message=FALSE}
...
```
```{r, warning=FALSE, message=FALSE}
waic_results <- waic(stanmodel_3)
print(waic_results)
```
By refitting the model using the Bayesian Approach(stan_glmer), we get similar MSE result with glmmTMB, confirming that model 3 is the best model. Also, we used WAIC to assess the goodness of fit of this Bayesian version of model 3. WAIC stands for Widely Applicable Information Criterion, it directly utilizes the posterior distribution of the Bayesian model parameters to calculate the expected log predictive density, hence, making it a robust metric of how well the model generalizes to new data. Additionally, compared with AIC and BIC with more restrictive assumptions, WAIC works better with non-linear and hierarchical models.
...
# Flatten the p-value matrix to a vector for adjustment
raw_p_values <- p_value_matrix[lower.tri(p_value_matrix)]

# Adjust p-values using the Benjamini-Hochberg method
adjusted_p_values <- p.adjust(raw_p_values, method = "BH")

# Reconstruct the adjusted p-value matrix
adjusted_p_value_matrix <- matrix(NA, nrow = n, ncol = n, dimnames = list(colnames(flipped_table), colnames(flipped_table)))
adjusted_p_value_matrix[lower.tri(adjusted_p_value_matrix)] <- adjusted_p_values
adjusted_p_value_matrix <- t(adjusted_p_value_matrix)
adjusted_p_value_matrix[lower.tri(adjusted_p_value_matrix)] <- adjusted_p_values
diag(adjusted_p_value_matrix) <- 1

# Print the adjusted p-value matrix
print("Adjusted P-Value Matrix(Benjamini-Hochberg method):")
print(round(adjusted_p_value_matrix, 4))
...
summary(newmodel.3)
```
According to the previous evaluation, we choose our new model 3 as the best model to interpret our analysis. The interpretation of model's output is written below:

For the random effect, the standard deviation of random intercept of 71 name groups is 0.2689, and the variance is 0.0723.

The residuals, which is the unexplained variation after accounting for fixed and random effects, have standard deviation 0.4204 and variance 0.1768.

For coefficients of the fixed effects, I will only interpret those variables that are statistically significant:

**Age6(-0.1974)**: Holding all other variables constant, compared with age group 1, the log of mean diversity score of age group 6 is expected to decrease by 0.1974.

**Age7(-0.2507)**: Holding all other variables constant, compared with age group 1, the log of mean diversity score of age group 7 is expected to decrease by 0.2507.

**SexM(-0.3857)**: Holding all other variables constant, compared with female orangutans, the  log of mean diversity score of male orangutans is expected to decrease by 0.3857.

**Minutes Awake(0.0002346)**: Holding all other variables constant, one more minutes awake for orangutan, its log of diversity score is expected to increase by 0.0002346.

For the coefficient of the interaction term, I will only interpret one of them because all the interpretation are very similar.

**Age7:SexM(0.6123)**: Holding all other variables constant, for age group 7, the log of mean diversity score is expected to have an additional increase of 0.6123 for male orangutan group compared with female orangutan group. In other words, in age group 7, the difference of log of mean diversity score between male and female group is further reduced by 0.6123.
...
In conclusion, this report mainly provides statistical insights, including data exploration, model assumption check and model optimizing. The best model suggests that orangutan's age group 6, group 7, orangutan's sex, the interaction term between age and sex, along with total minutes awake are key factors influencing behavioral diversity in orangutans. In addition, we also interpret several areas for future optimization and enhance the analysis.
...
## Model Diagnostics for Our Model 3
### New Model 3
